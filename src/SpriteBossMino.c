...
void START(){
    ...
    mino_v_coll = 0;
    mino_jump_power = 0u;
    mino_skull = SpriteManagerAdd(SpriteBossMinoskull, THIS->x, THIS->y);
        ...
        case ENEMY_PREATTACK:
            ...
            if(minotaur_data->wait == 70 || minotaur_data->wait == 10){
                if(THIS->mirror == NO_MIRROR){
                    SpriteManagerAdd(SpriteBossMinobuff, THIS->x + 24u, THIS->y + 12u);
                }else{
                    Sprite* s_minosbuff = SpriteManagerAdd(SpriteBossMinobuff, THIS->x - 6u, THIS->y + 12u);
